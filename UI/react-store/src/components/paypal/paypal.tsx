...
const PaypalComponent: React.FC<any> = (props: any) => {
    const advancedFraudTools = "true";

    const { grandTotal, grandTotalCurrency } = props.address;
    const { addressId, firstName, lastName, addressLine, city, state, country, zipCode, phone1 } = props.address.shipping;
    ...
    function setDeviceSessionData(clientInstance) {
        if (advancedFraudTools == "true") {
            braintree.dataCollector.create({
                ...
                const deviceData = JSON.parse(dataCollectorInstance.deviceData);
